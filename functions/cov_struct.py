...
class CovStruct:
    ...
    def optimalV_invsqrt(self, index, states_stacked, expected_next_states_stacked, 
                         action_index = None, gamma=0.9):
        V, isvar = self.covariance_matrix(index)
        optimal_V_inv = states_stacked @ np.linalg.inv(states_stacked.T@ states_stacked) @ (states_stacked - gamma * expected_next_states_stacked).T @ np.linalg.inv(V)
...
class Independence(CovStruct):
    ...
    def covariance_matrix(self, index):
        dim = self.model.N*self.model.cluster_T[index]
...
#%% Independence_sm
class Independence_sm(statsmodels.genmod.cov_struct.CovStruct):
    """
    An independence working dependence structure.
    """
    def __init__(self, gamma=0.9):

        super(Independence_sm, self).__init__()
    ...
    def setparams(self, GEE_model):
        ...
        self.cluster_T = GEE_model.cluster_T
        self.cluster_size = GEE_model.cluster_size
        self.action_indices = GEE_model.action_indices

    def covariance_matrix(self, index):
        dim = self.cluster_T[index]*self.cluster_size[index]
        return np.eye(dim, dtype=np.float64), True
    ...
    def covariance_matrix_solve(self, expval, index, stdev, rhs):
        ...
        if self.GEE_model.expected_next_states_action_list[self.action_index][index] is not None:
            D = (self.GEE_model.X_action_list[self.action_index][index] - self.gamma * self.GEE_model.expected_next_states_action_list[self.action_index][index]).T
            rslt = [D @ x for x in rslt]
        return rslt
...
class Exchangeable(CovStruct):
    ...
    def update(self):
        if self.model.MC_rho is None:
            ...
            nobs = np.sum(np.array(self.model.cluster_T) * np.array(self.model.cluster_size))
    ...
    def covariance_matrix(self, index, action_index = None):
        if action_index is None:
            dim = self.model.cluster_size[index] * self.model.cluster_T[index]
        ...
        R = (1 -  self.dep_params) * np.identity(dim) + self.dep_params #* np.ones([n, n])
...
class Exchangeable_sm(statsmodels.genmod.cov_struct.CovStruct):
    ...
    def covariance_matrix_solve(self, expval, index, stdev, rhs):
        ...
        """
        if self.GEE_model.expected_next_states_action_list[self.action_index][index] is None:
            ...
            epsilon = 1e-5  # Small value to add to the diagonal
            
            try:
                # Original operation that may raise a LinAlgError due to a singular matrix
                optimal_V_inv = self.GEE_model.X_action_list[self.action_index][index] @ np.linalg.inv(self.GEE_model.X_action_list[self.action_index][index].T @ self.GEE_model.X_action_list[self.action_index][index]) @ (self.GEE_model.X_action_list[self.action_index][index] - self.gamma * self.GEE_model.expected_next_states_action_list[self.action_index][index]).T @ np.linalg.inv(vmat)
            except np.linalg.LinAlgError:
                # Handling the singular matrix case by adding epsilon to the diagonal
                print("Singular matrix encountered, adjusting with epsilon.")
                matrix_to_invert = self.GEE_model.X_action_list[self.action_index][index].T @ self.GEE_model.X_action_list[self.action_index][index]
                adjusted_matrix = matrix_to_invert + epsilon * np.eye(matrix_to_invert.shape[0])
                # Attempting the operation again with the adjusted matrix
                optimal_V_inv = self.GEE_model.X_action_list[self.action_index][index] @ np.linalg.inv(adjusted_matrix) @ (self.GEE_model.X_action_list[self.action_index][index] - self.gamma * self.GEE_model.expected_next_states_action_list[self.action_index][index]).T @ np.linalg.inv(vmat)

            # optimal_V_inv = self.GEE_model.X_action_list[self.action_index][index] @ np.linalg.inv(self.GEE_model.X_action_list[self.action_index][index].T@ self.GEE_model.X_action_list[self.action_index][index]) @ (self.GEE_model.X_action_list[self.action_index][index] - self.gamma * self.GEE_model.expected_next_states_action_list[self.action_index][index]).T @ np.linalg.inv(vmat)
    
            soln = [optimal_V_inv @ x for x in rhs]
            return soln
...
class Exchangeable_subjects(CovStruct):
    ...
    def update(self):
        if self.model.MC_rho is None:
            ...
            for m in range(self.model.m):
                ...
                N += self.model.cluster_size[m] * self.model.cluster_T[m]
                df += 0.5 * self.model.cluster_size[m] * (self.model.cluster_size[m] - 1) * self.model.cluster_T[m]
    ...
    def covariance_matrix(self, index, action_index = None):
        off_block = self.dep_params * np.identity(self.model.cluster_T[index])
        # Rm = (1 - self.dep_params) * np.identity(self.model.cluster_size[index] * self.model.cluster_T[index]) + \
        #      np.kron(np.ones((self.model.cluster_size[index], self.model.cluster_size[index])),off_block)
        Rm = np.kron(np.ones((self.model.cluster_size[index], self.model.cluster_size[index])),off_block)
...
class Exchangeable_subjects_sm(statsmodels.genmod.cov_struct.CovStruct):
    ...
    def setparams(self, model):
        ...
        # print('cov',model)
        self.model = model
        self.cluster_T = model.cluster_T
    ...
    def covariance_matrix(self, endog, index=None):
        # Calculate the covariance matrix based on the correlation matrix
        # index: optional grouping variable indices
        off_block = self.dep_params * np.identity(self.cluster_T[index])
        # Rm = (1 - self.dep_params) * np.identity(self.cluster_size[index] * self.cluster_T[index]) + \
        #      np.kron(np.ones((self.cluster_size[index], self.cluster_size[index])),off_block)
        Rm = np.kron(np.ones((self.cluster_size[index], self.cluster_size[index])),off_block)
        np.fill_diagonal(Rm, 1)
...
class Autoex(CovStruct):
        ...
        if need_update:
                ...
                for m in range(self.model.m):
                    ...
                    for i1 in range(self.model.cluster_size[m]-1):
                        ...
                        for i2 in range(i1+1, self.model.cluster_size[m]):
                            ...
                            ssq += t1 * t2
                    df+= self.model.cluster_T[m]*self.model.cluster_T[m]*self.model.cluster_size[m]*(self.model.cluster_size[m]-1)/2
                if df >0:    
                    ...
                    if self.verbose: print('phi_2', phi_2, 'var_epsilon_2', var_epsilon_2, 'var_alpha_2', var_alpha_2,'var_td_2',var_td_2)
            ...
            # assume var(TD)=0
                ...
                if df >0:    
                    ...
                    self.set_weight =1
                if self.set_weight == None:
                    # df1=N_total*np.sum(self.model.cluster_T)
                    weight = df_1/(df_1+df)
                else:
                    weight = self.set_weight
                if df==0:
    ...
    def covariance_matrix(self, index, action_index=None):
        ...
        R = np.zeros((self.model.cluster_T[index], self.model.cluster_T[index]), dtype=np.float32)
        for i in range(self.model.cluster_T[index]):
            for j in range(self.model.cluster_T[index]):
        ...
        R = R + self.dep_params['var_alpha']  #np.ones(self.model.cluster_size[index] * self.model.cluster_T[index]) * self.dep_params['var_alpha'] 
        diag_indices = np.diag_indices_from(R)
        R[diag_indices] += self.dep_params['var_td']
        # R = R + np.eye(self.model.cluster_size[index] * self.model.cluster_T[index]) * self.dep_params['var_td']
...
class Autoex_sm(statsmodels.genmod.cov_struct.CovStruct):
    ...
    #     R = np.zeros((self.cluster_T[index],self.cluster_T[index]), dtype=np.float32)
    #     for i in range(self.cluster_T[index]):
    #         for j in range(self.cluster_T[index]):
    #             R[i,j] = self.dep_params["autoregressive_coef"] ** abs(i - j) * self.dep_params['var_epsilon/1-phi^2'] 
                   
    #     R = np.kron(np.eye(self.cluster_size[index]), R)
    #     R = R + np.ones(self.cluster_size[index] * self.cluster_T[index]) * self.dep_params['var_alpha'] 
    #     R = R + np.eye(self.cluster_size[index] * self.cluster_T[index]) * self.dep_params['var_td']
    #     R = R/(self.dep_params['var_epsilon/1-phi^2'] + self.dep_params['var_alpha']+self.dep_params['var_td'])
    ...
    def covariance_matrix(self, endog, index=None):
        ...
        R = np.kron(np.eye(self.cluster_size[index]), R)
        R = R + self.dep_params['var_alpha']  #np.ones(self.cluster_size[index] * self.cluster_T[index]) * self.dep_params['var_alpha'] 
        diag_indices = np.diag_indices_from(R)
        R[diag_indices] += self.dep_params['var_td']
        # R = R + np.eye(self.cluster_size[index] * self.cluster_T[index]) * self.dep_params['var_td']
        R = R/(self.dep_params['var_epsilon/1-phi^2'] + self.dep_params['var_alpha']+self.dep_params['var_td'])
...
class Autoex_old(CovStruct):
    ...
    '''
    def __init__(self, var_td=0, set_weight=None):
        super(Autoex_old, self).__init__()
        # dep_params = namedtuple("dep_params", ["autoregressive_part", "exchangeable_part"])
        self.dep_params = {"var_alpha":0,  "var_epsilon/1-phi^2":0, "autoregressive_coef":0, 'var_td':0}
        self.var_td=var_td
        self.set_weight = set_weight
...
#%% autoex_sm_old
class Autoex_sm_old(statsmodels.genmod.cov_struct.CovStruct):
    def __init__(self):

        super(Autoex_sm_old, self).__init__()
        # dep_params = namedtuple("dep_params", ["autoregressive_part", "exchangeable_part"])
        self.dep_params = {"autoregressive_part":0,  "exchangeable_part":0}
...
class Autoexsubject(CovStruct):
    ...
    '''
    def __init__(self, var_td=0, set_weight=None):
        super(Autoexsubject, self).__init__()
        # dep_params = namedtuple("dep_params", ["autoregressive_part", "exchangeable_part"])
        self.dep_params = {"var_alpha":0,  "var_epsilon/1-phi^2":0, "autoregressive_coef":0, 'var_td':0}
        self.var_td=var_td
        self.set_weight = set_weight
    ...
    def update(self):
        ...
        if need_update:
            ...
            if self.var_td:
                ...
                self.dep_params["var_alpha"] = self.model.MC_rho["var_alpha"] if self.model.MC_rho is not None and self.model.MC_rho['var_alpha'] is not None else residual_product / df
                self.dep_params["autoregressive_coef"] = self.model.MC_rho['autoregressive_coef'] if self.model.MC_rho is not None and self.model.MC_rho['autoregressive_coef'] is not None else lag2/lag1 
                self.dep_params["var_epsilon/1-phi^2"] = self.model.MC_rho['var_epsilon/1-phi^2'] if self.model.MC_rho is not None and self.model.MC_rho['var_epsilon/1-phi^2'] is not None else lag1/self.dep_params["autoregressive_coef"]
                self.dep_params["var_td"] = self.model.MC_rho["var_td"] if self.model.MC_rho is not None and self.model.MC_rho['var_td'] is not None else lag0 - self.dep_params["var_epsilon/1-phi^2"] - self.dep_params["var_alpha"]
                # if self.verbose:
                #     print('phi_1', phi_1, 'var_epsilon_1', var_epsilon_1, 'var_alpha_1', var_alpha_1, 'var_td_1',var_td_1)
    ...
    def covariance_matrix(self, index, action_index=None):
        ...
        # exsubject part
        off_block = self.dep_params["var_alpha"] * np.identity(self.model.cluster_T[index])
        R += np.kron(np.ones((self.model.cluster_size[index], self.model.cluster_size[index])),off_block)
        R = R + np.eye(self.model.cluster_size[index] * self.model.cluster_T[index]) * self.dep_params['var_td']
        R = R/(self.dep_params['var_epsilon/1-phi^2'] + self.dep_params['var_alpha']+ self.dep_params['var_td'])
        ...
        return R, False
#%% autoexsubject_sm
class Autoexsubject_sm(statsmodels.genmod.cov_struct.CovStruct):
    def __init__(self):

        super(Autoexsubject_sm, self).__init__()
        # dep_params = namedtuple("dep_params", ["autoregressive_part", "exchangeable_part"])
        self.dep_params = {"autoregressive_part":0,  "exchangeable_part":0}
    ...
    def covariance_matrix(self, endog, index=None):
        ...
        # exsubject part
        off_block = self.dep_params["var_alpha"] * np.identity(self.cluster_T[index])
        R += np.kron(np.ones((self.cluster_size[index], self.cluster_size[index])), off_block)
    ...
    def update(self, params):
        pass
#%% r~autoex and s ~ exchangeablesubject
class Autoex_exsubject(CovStruct):
    def __init__(self, var_td=0, set_weight=None):
        super(Autoex_exsubject, self).__init__()
        # dep_params = namedtuple("dep_params", ["autoregressive_part", "exchangeable_part"])
        self.dep_params = {"var_alpha":0,  "var_epsilon/1-phi^2":0, "autoregressive_coef":0, 'var_td':0, 'cov_s':0}
        self.var_td=var_td
        self.set_weight = set_weight
    ...
    def load_TD(self):
        if self.model.global_TD:
            self.model_m = self.model.global_m
            self.TD_list = self.model.global_TD_list
            self.marginal_variance = self.model.global_marginal_variance
            self.cluster_size = self.model.global_cluster_size
            self.cluster_T = self.model.global_cluster_T
        else:
            self.model_m = self.model.m
            self.TD_list = self.model.TD_list
            self.marginal_variance = self.model.marginal_variance
            self.cluster_size = self.model.cluster_size
            self.cluster_T = self.model.cluster_T
    ...
    def update(self):
        ...
        if need_update:
            ...
            if self.var_td:
                lag0, lag1, lag2, lag3 = 0.0, 0.0, 0.0, 0.0
                df_1=0
                for m in range(self.model_m):
                    TD_cluster_split = np.split(self.TD_list[m]/self.marginal_variance[m], self.cluster_size[m])
                    ...
                    for i1 in range(self.cluster_size[m]):
                        ...
                        if n > 3:
                            ...
                            lag0 += np.sum(resid**2) / n
                        df_1 += n
                N_total =np.sum(self.cluster_size)     
                if lag0 == 0.0:
                    raise ValueError("The sample size(T) is too small! Please try another correlation structure.")
                lag0 /= N_total
                lag1 /= N_total
                ...
                for m in range(self.model_m):
                    ...
                    # Compute residual product
                    for i1 in range(self.cluster_size[m]-1):
                        t1 = np.sum(TD_cluster_split[i1])
                        for i2 in range(i1+1, self.cluster_size[m]):
                            t2 = np.sum(TD_cluster_split[i2])
                            ssq += t1 * t2 - np.sum(TD_cluster_split[i1]*TD_cluster_split[i2])
                    df+= (self.cluster_T[m]*self.cluster_T[m] - self.cluster_T[m])*self.cluster_size[m]*(self.cluster_size[m]-1)/2
                if df >0:    
                    var_alpha_2 = self.model.MC_rho["var_alpha"] if self.model.MC_rho is not None and self.model.MC_rho['var_alpha'] is not None else ssq/df
                    phi_2 = self.model.MC_rho['autoregressive_coef'] if self.model.MC_rho is not None and self.model.MC_rho['autoregressive_coef'] is not None else (lag2-var_alpha_2)/(lag1 - var_alpha_2)
                    var_epsilon_2 = self.model.MC_rho['var_epsilon/1-phi^2'] if self.model.MC_rho is not None and self.model.MC_rho['var_epsilon/1-phi^2'] is not None else  (lag1-var_alpha_2)/phi_2 
            ...
            # assume var(TD)=0
                ...
                for m in range(self.model_m):
                    ...
                    for i1 in range(self.cluster_size[m]-1):
                        ...
                        for i2 in range(i1+1, self.cluster_size[m]):
                            t2 = np.sum(TD_cluster_split[i2])
                            ssq += t1 * t2
                    df+= self.cluster_T[m]*self.cluster_T[m]*self.cluster_size[m]*(self.cluster_size[m]-1)/2
                ...
                if df >0:    
                    ...
                    self.set_weight =1
                if self.set_weight == None:
                    # df1=N_total*np.sum(self.cluster_T)
                    weight = df_1/(df_1+df)
                else:
                    weight = self.set_weight
                if df==0:
            ...
            for m in range(self.model_m):
                ...
                df += 0.5 * self.cluster_size[m] * (self.cluster_size[m] - 1) * self.cluster_T[m]
           
            cov_s_plus_var_alpha = residual_product /df
            self.dep_params["cov_s"] = cov_s_plus_var_alpha - self.dep_params["var_alpha"]
        else:
            self.dep_params = self.model.MC_rho
    ...
    def covariance_matrix(self, index, action_index=None):
        ...
        ## correlation of the s_Exsubject part
        off_block = self.dep_params["cov_s"] * np.identity(self.cluster_T[index])
        R_ex = np.kron(np.ones((self.cluster_size[index], self.cluster_size[index])),off_block)
        np.fill_diagonal(R_ex, 0)
        R += R_ex
        ...
        return R, False
#%% r~autoex and s ~ exchangeablesubject sm
class Autoex_exsubject_sm(statsmodels.genmod.cov_struct.CovStruct):
    def __init__(self, optimal_GEE=False, gamma=0.9):

        super(Autoex_exsubject_sm, self).__init__()
        # dep_params = namedtuple("dep_params", ["autoregressive_part", "exchangeable_part"])
        self.dep_params = {"autoregressive_part":0,  "exchangeable_part":0}
